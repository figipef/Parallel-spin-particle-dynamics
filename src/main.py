...
P.plot_lots_v_time([v[:,0], v[:,1], v[:,2]], d['TIME_STEP'], Title="Momentum vs time", ylabel="Mom", Grid=True, Lims=((0.,d['TOTAL_TIME']),(np.min([v[:, 0], v[:, 1], v[:, 2]])*1.1,np.max([v[:, 0], v[:, 1], v[:, 2]])*1.1)), labels=["px", "py","pz"])
