...
def Field(t, L, E0):
    return np.where(((-10000 <= t/L) & (t/L <= 100000)), E0 * np.cos(t) * np.sin(np.pi * t / L), 0.)

def spin_in_x_E_in_y_linField(t, x, L, E0):
    E = Field(t-x, L, E0)
    return np.array([np.cos((0.00116)*E*0.3), np.sin((0.00116)*E*0.3), np.zeros(len(t))])
...
def plot_spin_v_sol(Q, dt, Title = "Spin v Time", ylabel = "relative amplitude", Grid = True, Lims = 0, labels = ["Spin x", "Spin y", "Spin z", "Spin theory x", "Spin theory y", "Spin theory z"], space_axis="Time [$1/\omega_c$]"): #takes and array of arrays q of some quantities and the time step between values and plots the data
    N = len(Q)    
    plt.figure(figsize=(8,5))
    ...
    for i in range(1):
        ...
        n = len(q)
        t = np.linspace(0., dt*n, n)
        s = spin_in_x_E_in_y_linField(t, np.zeros(len(t))+155, 125, 1)[i]
        #print(s)

        #plt.plot(t, q, label = labels[i])
        plt.plot(t, s, label = labels[N+i], linestyle = '--')


    plt.xlabel(space_axis, fontsize=16) 
    plt.ylabel(ylabel, fontsize=16)
    plt.title(Title, fontsize=18)
    plt.legend(fontsize=15)
